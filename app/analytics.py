...
class AnalyticsEngine:
    ...
    async def _calc_llm_impact(self, closed_trades: list) -> dict:
        ...
        if not llm_trades:
            return {"total": 0, "pass_rate": 0.0, "pass_avg_r": 0.0, "fail_avg_r": 0.0, "waitt_rate": 0.0, "model_usage": {}}
    ...
    def _calc_data_sufficiency(self, closed_trades: list) -> dict:
        """Calculate data sufficiency badges."""
        count = len(closed_trades)
        return {
            "trade_count": count,
            "statistical_significance": "HIGH" if count >= 250 else ("MEDIUM" if count >= 100 else ("LOW" if count >= 50 else "INSUFFICIENT")),
