    ...
    try:
        ...
        while True:
            ...
            try:
                ...
                # Send ping heartbeat
                await websocket.send_text(json.dumps({"type": "HEARTBEAT", "time": datetime.datetime.now(datetime.timezone.utc).isoformat()}))
...
async def update_strategy(payload: StrategyUpdateRequest):
    async with AsyncSessionLocal() as session:
        ...
        strat.version += 1
        strat.updated_at = datetime.datetime.now(datetime.timezone.utc)
        await session.commit()
