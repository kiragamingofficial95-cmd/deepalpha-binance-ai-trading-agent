...
class PaperTradingEngine:
        ...
        async with AsyncSessionLocal() as session:
            ...
            if stop_loss_price and take_profit_price:
                ...
                if actual_rr < self.MIN_EXECUTABLE_RR:
                    ...
                    # Return the USDT (minus fees already deducted)
                    usdt_bal.free += amount_usdt
    ...
    async def close_position(
        ...
        async with AsyncSessionLocal() as session:
            ...
            trade.exit_reason_enum = exit_reason
            trade.status = "CLOSED"
            trade.pnl = pnl
    ...
    async def evaluate_open_positions_triggers(self) -> list[dict[str, Any]]:
        ...
        async with AsyncSessionLocal() as session:
            ...
            for trade in open_trades:
                ...
                if should_close:
                    await session.commit()  # Save any highest_price/lowest_price/MFE/MAE changes before close
                    result = await self.close_position(trade.id, exit_reason=close_reason, custom_exit_price=curr_price)
                    if result.get("success"):
                        # Update exit_reason_enum on the returned trade
