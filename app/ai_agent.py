...
# Dedicated ICT VALIDATOR prompt. The LLM is ONLY a validator of the deterministic
# strategy output — it must never design trades or invent market data.
ICT_VALIDATOR_PROMPT = """You are validating the deterministic strategy output. You are not designing a trade.

You receive a fully computed ICT / Market Mechanics setup produced by a deterministic Python
strategy engine (4H macro, 1H bias, 15M POI, 5M entry confirmation, A+ checklist, SL/TP, RR).

Your ONLY job: decide whether the provided setup actually satisfies the strategy below.

STRICT RULES:
- Use ONLY the data provided. You MUST NOT invent market data, indicators, POIs, liquidity
  sweeps, MSS/BOS, RR, SL or TP. You cannot and must not propose alternative entries, stops,
  targets, position sizes or risk values — those are fixed by the deterministic engine.
- If any information required to judge a condition is absent or ambiguous, choose FAIL or WAIT.
  Never assume missing data is favorable.
- decision must be exactly one of PASS, FAIL, WAIT:
  * PASS  = every required condition is evidenced in the provided data.
  * FAIL  = the setup clearly violates a strategy rule.
  * WAIT  = information is missing/ambiguous or confirmation is not yet complete.
- confidence must be a number between 0.0 and 1.0.
- Return ONLY a JSON object: {\"decision\": \"PASS|FAIL|WAIT\", \"confidence\": 0.0, \"reasoning\": \"...\", \"concerns\": [\"...\"]}

=== STRATEGY SUMMARY (what you validate against) ===
- 4H: macro sanity — trade must not fight the macro trend (EMA20/EMA50 context).
- 1H: clear directional bias with invalidation level (swings, EQ, volume profile). Unclear bias = no trade.
- 15M: ONE clean unmitigated POI (Order Block or FVG) with volume-profile confluence.
- 5M: objective entry confirmation — liquidity sweep and/or MSS/BOS, or bias-direction reversal candle close.
- A+ checklist: 1H bias agreement, clean POI, 5M confirmation, valid setup time, minimum 2R to TP1.
- No session/timezone restriction applies.
"""
...
class AIAgent:
    ...
    async def _execute_tool(self, name: str, args: dict[str, Any]) -> dict[str, Any]:
        ...
        if name == "analyze_trade_history":
            ...
            async with AsyncSessionLocal() as session:
                ...
                strategy.version += 1
                strategy.updated_at = datetime.datetime.now(datetime.timezone.utc)
                await session.commit()
                return {
                    "success": True,
    ...
    async def evaluate_ict_setup_with_llm(self, symbol: str, ict_state: dict[str, Any]) -> dict[str, Any]:
        ...
        FAIL-CLOSED POLICY: if the LLM is unavailable, times out, returns malformed or
        ambiguous output, the decision is FAIL (or WAIT for transient rate limits) —
        the trade is NOT opened. A missing API key never yields PASS.
        ...
        # Check cache (deterministic key so identical state hits regardless of process)
        cache_key = hashlib.sha256(
            f"{symbol}_{json.dumps(ict_state, sort_keys=True, default=str)}".encode("utf-8")
        ).hexdigest()
        cached = llm_cache.get(cache_key)
        if cached and now - cached.get("_cached_at", 0) < llm_cache_ttl:
            logger.debug(f"LLM cache hit for {symbol}")
            return cached

        client = await self.get_client()
        if not client:
            # FAIL CLOSED: no API key configured → the LLM gate cannot validate → no trade.
            return {
                "decision": "FAIL",
                "confidence": 0.0,
                "llm_reasoning": "LLM validator unavailable: Groq API key not configured. Failing closed — no trade.",
        ...
        prompt = (
            ...
            f"4H: {json.dumps(ict_state.get('macro_4h', {}), default=str)}\n"
            f"1H: {json.dumps(ict_state.get('bias_1h', {}), default=str)}\n"
            f"15M POI: {json.dumps(ict_state.get('poi_15m', {}), default=str)}\n"
            f"5M Entry: {json.dumps(ict_state.get('entry_5m', {}), default=str)}\n"
            f"A+ Checks: {json.dumps(ict_state.get('a_plus', {}), default=str)}\n"
        ...
        try:
            ...
            # Hard timeout — a hung LLM call must never wedge the bot cycle or pass by default
            resp = await asyncio.wait_for(
                client.chat.completions.create(
                    model=target_model,
                    messages=[
                        {"role": "system", "content": ICT_VALIDATOR_PROMPT},
            ...
            # Parse and strictly validate JSON response
            try:
                result = json.loads(content)
            except json.JSONDecodeError:
                logger.warning(f"LLM returned invalid JSON for {symbol}: {content!r}")
                return {"decision": "FAIL", "confidence": 0.0, "llm_reasoning": "Invalid JSON response from LLM", "concerns": ["LLM_MALFORMED"], "model": target_model}

            decision = result.get("decision")
            if not isinstance(decision, str):
                decision = None
            decision = decision.upper().strip() if decision else None
            if decision not in ("PASS", "FAIL", "WAIT"):
                # Missing/ambiguous decision → fail closed
                logger.warning(f"LLM returned missing/ambiguous decision for {symbol}: {result!r}")
                return {"decision": "FAIL", "confidence": 0.0, "llm_reasoning": f"Missing or ambiguous decision in LLM response: {decision!r}", "concerns": ["LLM_AMBIGUOUS"], "model": target_model}
            ...
            # Bound the cache so it cannot grow indefinitely
            llm_cache[cache_key] = cached_result
            if len(llm_cache) > LLM_CACHE_MAX_ENTRIES:
                # Drop the oldest entries
                for k in sorted(llm_cache, key=lambda k: llm_cache[k].get("_cached_at", 0))[: len(llm_cache) - LLM_CACHE_MAX_ENTRIES]:
                    llm_cache.pop(k, None)

            return cached_result

        except asyncio.TimeoutError:
            logger.warning(f"LLM validation timed out for {symbol} after {settings.LLM_VALIDATION_TIMEOUT_SECONDS}s")
            return {"decision": "FAIL", "confidence": 0.0, "llm_reasoning": f"LLM validation timed out after {settings.LLM_VALIDATION_TIMEOUT_SECONDS}s", "concerns": ["LLM_TIMEOUT"], "model": "timeout"}
        except Exception as e:
            error_str = str(e)
            logger.warning(f"Groq ICT LLM validation warning: {e}")

            # Handle rate limit (429) — transient, WAIT (blocked this cycle, retried later)
            if "429" in error_str or "rate_limit" in error_str.lower() or "Rate limit" in error_str:
                ...
                return {"decision": "WAIT", "confidence": 0.0, "llm_reasoning": f"Rate limited. Try again in {wait_sec:.0f}s", "concerns": ["LLM_RATE_LIMIT"], "model": "rate_limited"}

            # Any other LLM failure → FAIL CLOSED
            return {"decision": "FAIL", "confidence": 0.0, "llm_reasoning": f"LLM error: {error_str}", "concerns": ["LLM_ERROR"], "model": "error"}
