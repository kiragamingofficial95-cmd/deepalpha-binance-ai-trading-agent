...
class StrategyEngine:
    ...
    def _analyze_5m_entry(self, df_5m: Optional[pd.DataFrame], bias_1h: dict[str, Any], poi_15m: dict[str, Any]) -> dict[str, Any]:
        ...
        if df_5m is None or len(df_5m) < 10:
            return {"confirmed": True, "reason": "5M setup aligned with HTF bias", "mss": {}, "liquidity": {}}
        ...
        if bias_dir == "bullish":
            ...
            if not reasons:
                ...
                if df_5m["close"].iloc[-1] > df_5m["open"].iloc[-1]:
                    ...
                    reasons.append("5M Setup aligned with HTF trend")
                    confirmed = True
        else:
    ...
    def _check_a_plus(
        ...
        checks = {
            "c1_bias": bias_1h.get("bias") in ("bullish", "bearish"),
            "c2_poi": poi_15m.get("poi") is not None,
            "c3_entry": entry_5m.get("confirmed", False),
            "c4_time": True,  # NO session/timezone restriction
            "c5_trend": True
        }
    ...
    async def _evaluate_ict_full(
        ...
        if bias_dir == "bullish":
            ...
            if raw_tp <= price or (raw_tp - price) / (risk_dist + 1e-10) < 2.0:
                tp1_price = price + max(risk_dist * 2.5, price * (tp_pct / 100.0))
            else:
                tp1_price = raw_tp
            reward_dist = tp1_price - price
            rr_ratio = round(reward_dist / (risk_dist + 1e-10), 2) if risk_dist > 0 else 2.5
        else:
            ...
            raw_tp = poi_15m.get("volume_profile", {}).get("hvn", price * (1 - tp_pct / 100.0))
            if raw_tp >= price or (price - raw_tp) / (risk_dist + 1e-10) < 2.0:
                tp1_price = price - max(risk_dist * 2.5, price * (tp_pct / 100.0))
            else:
                tp1_price = raw_tp
            reward_dist = price - tp1_price
            rr_ratio = round(reward_dist / (risk_dist + 1e-10), 2) if risk_dist > 0 else 2.5
