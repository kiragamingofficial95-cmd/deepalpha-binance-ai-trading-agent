...
class Trade(Base):
    ...
    entry_time = Column(DateTime, default=lambda: datetime.datetime.now(datetime.timezone.utc), index=True)
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "technical_snapshot": json.loads(self.technical_snapshot) if self.technical_snapshot else None,
            "fees_estimate": round(self.fees_estimate, 4) if self.fees_estimate else None,
            "slippage_estimate": round(self.slippage_estimate, 4) if self.slippage_estimate else None,
            "market_state_snapshot": json.loads(self.market_state_snapshot) if self.market_state_snapshot else None,
